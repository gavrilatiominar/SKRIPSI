        ...
        while (byteRead < (w * h)) {
            if (byteRead % 3 == 0) {
                ...
                if (byteRead % chunkSize == 0) {
                    for (int i = 0; i < 64; i++) {
                        this.ZR[i] = Math.round((this.XR[i] + this.YR[i]) / 2);
                    }
                    this.chiSquared[byteRead / chunkSize] = chi.chiSquareTest(ZR, XR);
                    for (int i = 0; i < 64; i++) {
                        this.ZG[i] = Math.round((this.XG[i] + this.YG[i]) / 2);
                    }
                    this.chiSquared[byteRead / chunkSize] = chi.chiSquareTest(ZG, XG);
                    for (int i = 0; i < 64; i++) {
                        this.ZB[i] = Math.round((this.XB[i] + this.YB[i]) / 2);
                    }
                    this.chiSquared[byteRead / chunkSize] = chi.chiSquareTest(ZB, XB);
                    for (int i = 0; i < 64; i++) {
                        this.XR[i] = 0;
                        this.YR[i] = 0;
                        this.ZR[i] = 0;
